...
def underwriter_deterministic(prop: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"price": price, "gpr": gpr, "egi": egi, "noi": noi, "operating_expenses": operating, "loan_amount": loan_amount, "annual_debt_service": annual_debt_service, "dscr": dscr, "refund": refund, "existing_debt": existing_debt, "equity_gap": equity_gap, "britton_score": britton_score, "confidence": confidence}
...
def monte_carlo_simulation(prop: Dict[str, Any], runs: int = 2000) -> Dict[str, Any]:
    ...
    return {"runs": runs, "dscr_p10": pct(dscr_samples, 10), "dscr_p25": pct(dscr_samples, 25), "dscr_p50": pct(dscr_samples, 50), "dscr_p75": pct(dscr_samples, 75), "dscr_p90": pct(dscr_samples, 90), "yield_p50": pct(yield_samples, 50), "noi_p50": pct(noi_samples, 50)}
...
# Verification pipeline & checks (stubs)
def verify_listing_pipeline(listing: Dict[str, Any], attempts: int = 5, require_checks: int = 5) -> Dict[str, Any]:
    ...
    passed = 0
    check_fns = [check_primary_listing_active, check_county_record_match, check_tax_lien_status, check_contact_validation, check_independent_third_source, check_title_snapshot, check_saved_search_crossref, check_agent_mls_validation, check_geocode_and_parcel, check_photo_and_image_for_condition]
    attempts_made = 0
    i = 0
