...
@app.route("/")
def index(): 
    return "<h1>Britton Method API</h1><p>Endpoints: /health, /analyze, /verify, /generate-loi, /waterfall/simulate, /webhook/email, /persona-stack, /diagnostics/run, /diagnostics/env</p>"
...
def loi_route():
    ...
    if not prop: return jsonify({"ok":False,"error":"missing property"}), 400
    try:
        loi_text = generate_long_loi_text(prop)
        return jsonify({"ok":True,"loi":loi_text})
    except Exception as e:
...
def job_status(job_id):
    ...
    if not job: return jsonify({"ok":False,"error":"unknown job_id"}), 404
    return jsonify({"ok":True,"job":job})

@app.route("/persona-stack", methods=["POST"])
def persona_stack_route():
    if API_KEY: require_api_key()
    payload = request.json or {}
    prop = payload.get("property", payload)
    if not prop: return jsonify({"ok":False,"error":"missing property"}), 400
    if assign_persona_stack: stack = assign_persona_stack(prop)
    else: stack = ["buyer", "seller", "analyst"]
    return jsonify({"ok":True,"persona_stack":stack})
...
def diagnostics_run():
    try:
        res = {
            ...
            "jobs_queue_size": JOB_QUEUE.qsize(),
            "env_summary": {k: mask_key(os.environ.get(k)) for k in ["BRITTON_API_KEY","OPENAI_API_KEY","ATTOM_API_KEY","TWILIO_SID","TWILIO_AUTH","SENTRY_DSN"]},
        }
        return jsonify({"ok":True,"diagnostics":res})
    except Exception as e:
        return jsonify({"ok":False,"error":str(e)}), 500

@app.route("/diagnostics/env")
def diagnostics_env():
    keys = ["BRITTON_API_KEY","OPENAI_API_KEY","ATTOM_API_KEY","TWILIO_SID","TWILIO_AUTH","SENTRY_DSN"]
    envs = {k: mask_key(os.environ.get(k)) for k in keys}
    return jsonify({"ok":True,"env":envs})

@app.route("/shutdown", methods=["POST"])
def shutdown():
    if API_KEY: require_api_key()
    JOB_QUEUE.put(None)
    return jsonify({"ok":True,"message":"Shutdown signal sent to worker."})
